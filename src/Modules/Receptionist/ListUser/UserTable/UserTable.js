...
export default function UserTable({ loading, updateList }) {
  ...
  //   const token = localStorage.getItem("token");
  //   await axios.get(`https://nmcnpm.herokuapp.com/api/v2/user/detail/` + id, userActivated, { headers: { "Authorization": `Bearer ${token}` } })
  //     .then(res => {
      ...
      <table className="contenttable">
        <thead>
          <tr>
            ...
            <th style={{ paddingRight: '80px' }}>Indentify number</th>
            ...
            posts.map((post, index) => {
              ...
              if (index >= (currentPage - 1) * 10 && index <= (currentPage - 1) * 10 + 9)
                return (<tr key={index}>
                  ...
                  <td style={{ paddingRight: '30px' }} >{post.balance}</td>
